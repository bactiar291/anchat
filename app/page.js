...
export default function Page() {
  ...
  useEffect(() => {
    fetch("/api/chat")
      .then(r => r.json())
      .then(d => {
        if (d.currentModel) setCurrentModel(d.currentModel.label);
      })
      .catch(() => setCurrentModel("Llama 3.3 · 70B"));
  }, []);
  ...
  const sendMessage = useCallback(async () => {
    ...
    // Build messages array untuk API (tanpa field time)
